...
router.get('/vendors', authenticate, authorize('ADMIN'), validatePagination, async (req, res) => {
  try {
...
// @access  Private (Admin)
router.get('/vendors/:id', authenticate, authorize('ADMIN'), validateObjectId('id'), async (req, res) => {
  try {
...
// @access  Private (Admin)
router.put('/vendors/:id/status', authenticate, authorize('ADMIN'), validateObjectId('id'), async (req, res) => {
  try {
    ...
    try {
      ...
      switch(status) {
        ...
        case 'REJECTED':
          ...
          message = `We regret to inform you that your vendor application has been rejected. ${notes ? 'Reason: ' + notes + '. ' : ''}Please contact support for more information or to reapply.`;
          break;
        case 'SUSPENDED':
          title = 'Account Suspended';
          message = `Your vendor account has been suspended. ${notes ? 'Reason: ' + notes + '. ' : ''}Please contact support for more information.`;
          break;
        default:
      ...
      console.log(`Notification created for vendor ${vendor.businessName} about status change to ${status}`);
      
      // TODO: Email notification will be added later once nodemailer is properly configured
      console.log(`Email notification would be sent to ${vendor.email} about status change to ${status}`);
    ...
    console.log(`Vendor ${vendor.businessName} status changed from ${oldStatus} to ${status}`);
...
// @access  Private (Admin)
router.get('/centers', authenticate, authorize('ADMIN'), validatePagination, async (req, res) => {
  try {
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 10;
    const skip = (page - 1) * limit;
...
// @access  Private (Admin)
router.put('/:id/activate', authenticate, authorize('ADMIN'), validateObjectId('id'), async (req, res) => {
  try {
