...
export function VendorDashboard({ user, onLogout }: VendorDashboardProps) {
  ...
  const mockOrders: Order[] = [
    ...
    },
    {
      ...
      vendorId: user.id,
      items: [
        { productId: 'prod-3', productName: 'Smart Watch', quantity: 3, price: 3500, total: 10500 }
      ],
      totalAmount: 10500,
      ...
      center: { id: 'center-3', name: 'Bangalore Tech Hub', email: 'bangalore@demo.com' } as any
    },
    {
      id: 'ORD-004',
      centerId: 'center-4',
      vendorId: user.id,
      items: [
        { productId: 'prod-4', productName: 'Laptop Stand', quantity: 5, price: 800, total: 4000 }
      ],
      totalAmount: 4000,
      ...
      center: { id: 'center-4', name: 'Chennai Operations', email: 'chennai@demo.com' } as any
    }
  ];
  ...
  const mockProducts: Product[] = [
    ...
    },
    {
      ...
      vendorId: user.id,
      name: 'USB-C Hub',
      description: 'Multi-port USB-C hub with HDMI, USB 3.0, and charging support',
      price: 1200,
      category: 'Accessories',
      stock: 5,
      status: 'available' as any,
  ...
  const mockNotifications = [
    { id: 1, title: 'Order Confirmed', message: 'Order ORD-001 confirmed by Delhi Distribution Center', type: 'success', time: '2 hours ago' },
    { id: 2, title: 'Low Stock Alert', message: 'USB-C Hub stock is running low (5 remaining)', type: 'warning', time: '4 hours ago' },
    { id: 3, title: 'Order Delivered', message: 'Order ORD-003 delivered successfully from Bangalore Tech Hub', type: 'success', time: '1 day ago' }
  ];
          ...
          <div className="space-y-6 bg-gradient-to-br from-cyan-50/30 to-blue-50/30 min-h-screen -m-6 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {stats.map((stat) => {
                ...
                return (
                  <Card key={stat.label} className="hover:shadow-xl transition-all duration-300 bg-gradient-to-br from-white to-cyan-50/50 border-cyan-200">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <div className="flex items-center mt-2">
                          ...
                          <span className={`text-sm font-medium ml-1 ${stat.trend === 'up' ? 'text-emerald-600' : 'text-red-600'}`}>
                            {stat.change}
                          </span>
            ...
            <Card className="bg-gradient-to-br from-white to-blue-50/50 border-blue-200 shadow-lg">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Button 
                  variant="secondary" 
                  icon={Plus} 
                  className="h-20 flex-col bg-gradient-to-r from-cyan-500 to-blue-600 text-white hover:from-cyan-600 hover:to-blue-700 border-0 shadow-lg hover:shadow-xl transition-all duration-300"
                >
                  <span className="mt-2">Place New Order</span>
                </Button>
                <Button 
                  variant="secondary" 
                  icon={BarChart3} 
                  className="h-20 flex-col bg-gradient-to-r from-blue-500 to-indigo-600 text-white hover:from-blue-600 hover:to-indigo-700 border-0 shadow-lg hover:shadow-xl transition-all duration-300"
                ...
                <Button 
                  variant="secondary" 
                  icon={Download} 
                  className="h-20 flex-col bg-gradient-to-r from-indigo-500 to-cyan-600 text-white hover:from-indigo-600 hover:to-cyan-700 border-0 shadow-lg hover:shadow-xl transition-all duration-300"
                >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2">
                <Card className="bg-gradient-to-br from-white to-cyan-50/50 border-cyan-200 shadow-lg">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="text-lg font-semibold text-slate-900">Recent Orders Placed</h2>
                    <Button variant="ghost" size="sm" onClick={() => setActiveTab('orders')} className="text-blue-600 hover:text-blue-700 hover:bg-blue-50">
                      View All
                    </Button>
                  </div>
                  <div className="space-y-4">
                    {mockOrders.slice(0, 4).map((order) => (
                      <div key={order.id} className="flex items-center justify-between p-4 bg-gradient-to-r from-cyan-50 to-blue-50 rounded-lg hover:from-cyan-100 hover:to-blue-100 transition-all duration-300 border border-cyan-200">
                        <div className="flex items-center space-x-4">
                          <div className={`w-3 h-3 rounded-full ${
                            order.status === OrderStatus.PENDING ? 'bg-amber-400' :
                            order.status === OrderStatus.CONFIRMED ? 'bg-blue-400' :
                            order.status === OrderStatus.COMPLETED ? 'bg-emerald-400' :
                            'bg-purple-400'
                          }`}></div>
                          <div>
                            <p className="font-medium text-slate-900">{order.id}</p>
                            <p className="text-sm text-slate-600">To: {order.center?.name}</p>
                            <p className="text-sm text-slate-500">Placed: {order.createdDate}</p>
                          </div>
                        </div>
              ...
              <div>
                <Card className="bg-gradient-to-br from-white to-blue-50/50 border-blue-200 shadow-lg">
                  ...
                  <div className="space-y-3">
                    {mockNotifications.map((notification) => (
                      <div key={notification.id} className="flex items-start space-x-3 p-3 bg-gradient-to-r from-cyan-50 to-blue-50 rounded-lg border border-cyan-200">
                ...
                <Card className="mt-6 bg-gradient-to-br from-white to-cyan-50/50 border-cyan-200 shadow-lg">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => setActiveTab('products')} className="text-blue-600 hover:text-blue-700 hover:bg-blue-50">
                      Browse
                    </Button>
                  </div>
                  <div className="space-y-3">
                    {mockProducts.slice(0, 3).map((product) => (
                      <div key={product.id} className="flex items-center space-x-3 p-2 rounded-lg hover:bg-gradient-to-r hover:from-cyan-50 hover:to-blue-50 transition-all duration-300">
                        <img 
                        ...
                        <div className="text-right">
                          <p className="text-sm font-medium text-slate-900">{product.stock}</p>
