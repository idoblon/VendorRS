...
export function ApplicationsComponent() {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const transformedApplications = applicationsData.map((item: any) => ({
          ...
          location: item.district ? `${item.district}, ${item.province || ''}` : undefined,
          email: item.email,
          phone: item.phone,
          status: item.status.toLowerCase() as "pending" | "approved" | "rejected",
          submittedDate: item.createdAt ? new Date(item.createdAt).toISOString().split('T')[0] : "",
          documents: item.documents ? item.documents.map((doc: any) => doc.originalName || "Document") : []
        }))
  ...
  useEffect(() => {
    console.log("useEffect triggered with activeTab:", activeTab, "and statusFilter:", statusFilter);
    fetchApplications()
