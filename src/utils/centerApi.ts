...
export const getCentersByCategory = async (
  category: string
): Promise<Center[]> => {
  try {
    const response = await axiosInstance.get(
      `/api/users/centers/category/${category}`
    );
...
export const getCenterCategories = async (): Promise<string[]> => {
  try {
    const response = await axiosInstance.get("/api/users/centers/categories", {
      headers: { Authorization: `Bearer ${localhost.getItem("vrs_token")}` },
    });
...
export const getAllCenters = async (params?: {
  ...
  try {
    ...
    console.log(
      "Admin centers endpoint not accessible, using category-based approach"
    );
  }
  ...
  try {
    ...
    const centersPromises = categories.map(async (category) => {
      try {
        ...
        console.warn(
          `Failed to fetch centers for category ${category}:`,
          error
        );
        return [];
      }
